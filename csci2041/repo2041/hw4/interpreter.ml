...
if Array.length Sys.argv < 2 then () else
  ...
  (* Parse the program...*)
  try (let progExpr = progString |> wordlist |> tokens |> parse_program in
  ...
  (* Run the program. *)
  let _ = eval optProg [] in ()) with ParseError (err,i) ->
  print_string ("Parsor error at line "^string_of_int(find_line progLines (List.length (progString|> wordlist |> tokens) - i))^" "^err^"\n" )
