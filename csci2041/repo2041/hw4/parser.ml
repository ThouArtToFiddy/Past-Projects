...
type token = OP | CP | AND | OR | NOT | PLUS | MINUS | TIMES | DIV | LET | ID of string | CONST of int | BCONST of bool | LT | GT | EQ | IF |
	     SET | SEQ | WHILE | PRINT |
	     APP | FUN | COLON | ARROW | INT | BOOL | UNIT | LB | RB | CONS | TAIL | HEAD | LIST | READ
...
let find_line slist i =
let rec line_h slist token_i line = match slist with
| [] -> line+1
| h::t -> if (token_i-(List.length (h|> wordlist |> tokens)))<0 then line else line_h t (token_i-(List.length (h|> wordlist |> tokens))) (line+1)
in line_h slist i 1
...
let rec _parse_type_expr tlist =
  let (ty1, tl) =
    ...
    | OP::t -> (match _parse_type_expr t with
                | ty, CP::t' -> (ty,t')
	              | _,t' -> failwith "imbalanced parentheses in type expression")
...
| [] -> raise (ParseError ("Ran out of tokens without closing parenthesis" ,0))
| (BCONST b)::t -> (BConst b,t)
| (CONST i)::t -> (Const i, t)
| (ID s)::t -> (Name s, t)

| (READ)::t -> (Readint, t)
| LB::t -> let (lst, t') = _parse_literal t [] in (LConst (List.rev lst), t')
...
| OP::CP::t -> (Seq [], t)
| x ->  raise ( ParseError ("Unexpected token: unbalanced parentheses or keyword out of call position", List.length x))


and _parse_literal lst acc =
let rec parse_list lst acc = match lst with
| RB::tail -> (acc,tail)
| (CONST i)::t -> _parse_literal t (i::acc)
| [] -> raise ( ParseError ("unclosed list literal", 0))
| x -> raise (ParseError ("list literal with non-int element", (List.length x)))
in parse_list lst acc
...
and _parse_two t =
  ...
  | x -> raise (ParseError( "parser: missing closing paren.", (List.length x)))

(* parse a list of expressions, consuming the final closing paren *)
and _parse_list t =  match t with
| CP::t' -> ([],t')
| [] -> raise (ParseError ("unfinished expression sequence: missing close paren(s).", 0))
