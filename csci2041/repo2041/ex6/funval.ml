...
let add1fun = Fun ("x", Int, Add(Const 1, Name("x")))

let collatz_fun = Fun ("x", Int, Let ("x", Const 31, Let ("x2", Div (Name "x", Const 2), If (Eq (Mul (Name "x2", Const 2), Name "x"), Name "x2", Add (Mul (Const 3, Name "x"), Const 1)))))

let kdelta = Fun ("e1", Bool, If(Eq(Fun("e2", Int, Add(Name "e2", Const 0)) , Name "e1"), Const 1, Const 0 ))

let rec subst expr = Const 1

let inline expr = subst expr
