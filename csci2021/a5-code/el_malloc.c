...
el_blockhead_t *el_block_below(el_blockhead_t *block){
  el_blockfoot_t *prevblockfoot = PTR_MINUS_BYTES(block, sizeof(el_blockfoot_t));
  if((void *) prevblockfoot <= (void*) el_ctl.heap_start){
    return NULL;
  }
  else{
    return el_get_header(prevblockfoot);
  }
}
...
void el_add_block_front(el_blocklist_t *list, el_blockhead_t *block){
  block->prev = list->beg;                            //update block
  block->next = list->beg->next;
  block->prev->next = block;
  block->next->prev = block;

  list->length++;                                    //update list
...
void el_remove_block(el_blocklist_t *list, el_blockhead_t *block){
    block->prev->next = block->next;            //update prev and next
...
el_blockhead_t *el_find_first_avail(size_t size){
    ...
    while (cur != NULL){
      if (cur->size >= size + EL_BLOCK_OVERHEAD){ //checks for sufficient size
        return cur;
      }
      cur = cur->next;
    }
...
el_blockhead_t *el_split_block(el_blockhead_t *block, size_t new_size){
  ...
  if(oldsize < new_size + EL_BLOCK_OVERHEAD){
    return NULL;
  }

  el_blockfoot_t *oldfoot = el_get_footer(block);    //saves the old footer of the block

  block->size = new_size;                            //updates size of block

  el_blockfoot_t *newfooter = el_get_footer(block); //create new footer
  newfooter->size = new_size;

  el_blockhead_t *newhead = PTR_PLUS_BYTES(newfooter, sizeof(el_blockfoot_t));  //create new smaller block
  newhead->size = oldsize - new_size - EL_BLOCK_OVERHEAD;
  oldfoot->size = newhead->size;
...
void *el_malloc(size_t nbytes){
  ...
  if (block == NULL) {                        //makes sure that find_first_avail actually finds something
    return NULL;
  }
  el_remove_block(el_ctl.avail,block);        //remove original

  el_blockhead_t *newhead = el_split_block(block,nbytes);       //split the available block

  block->state = EL_USED;                   //update state and lists
...
void el_merge_block_with_above(el_blockhead_t *lower){
  if(lower == NULL || lower->state != EL_AVAILABLE){ //cases to fail the return
    return;
  }
  ...
  el_remove_block(el_ctl.avail, higher);      //remove from available
  el_remove_block(el_ctl.avail, lower);

  lower->size = lower->size + higher->size + EL_BLOCK_OVERHEAD;   //update lower size
...
void el_free(void *ptr){
  ...
  el_remove_block(el_ctl.used, block);               //"frees" the block
  el_add_block_front(el_ctl.avail, block);

  block->state = EL_AVAILABLE;                       //change state

  el_blockhead_t *below = el_block_below(block);

  el_merge_block_with_above(block);                  //attempt to merge with above
  el_merge_block_with_above(below);                  //and below
