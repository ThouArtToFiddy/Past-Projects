...
int main(int argc, char *argv[]){
  ...
  while(1){
    ...
    if( strcmp("print", cmd)==0 ){   // print command
      if(echo){
        printf("print\n");
      }
    ...
    else if( strcmp("quit", cmd)==0 ){     // check for exit command
      if(echo){
        printf("quit\n");
      }
      break;                          // break from loop
      ...
      success = bst_insert(&tree, cmd); // call insert function
    ...
    else if( strcmp("save", cmd)==0 ){ // save file
      fscanf(stdin,"%s",cmd );
      bst_save(&tree,cmd);
      if(echo)
        printf("save %s\n",cmd);
     ...
     else if( strcmp("load", cmd)==0 ){ // load file
      fscanf(stdin,"%s",cmd );
      ...
      success = bst_load(&tree, cmd); // call load function
      if(!success){                      // check for success
        printf("load failed\n");
      }
    }
