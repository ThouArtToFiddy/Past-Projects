...
type expr =
  Const of int | BConst of bool | LConst of int list
  | Add of expr * expr
  | Sub of expr * expr
  | Mul of expr * expr
  | Div of expr * expr
...
let rec eval exp state = match exp with
  ...
  | Head e -> let (ListV v1, st) = eval e state in (IntV(List.hd v1), st)
  | Tail e -> let (ListV v1, st) = eval e state in (ListV(List.tl v1), st)
  ...
  | Print e -> let (r,st') = eval e state in
     ...
     | ListV l -> print_string ("["^(printlist l)^"]")
...
and evalCons e1 e2 state =
  let (IntV v1, st1) = eval e1 state in
  let (ListV v2, st2) = eval e2 st1 in
  (ListV(v1 ::v2),st2)
...
let rec typeof exp env = match exp with
  ...
  | Head e -> (match (typeof e env) with
    | ListT -> IntT
    | _ -> failwith "head of non-list"
    )
  | Tail e -> (match (typeof e env) with    (*e : ListT => Tail e : List*)
    | ListT -> ListT
    | _ -> failwith "tail of non-list"
    )
  | Cons (e1,e2) -> (match (typeof e1 env, typeof e2 env) with    (*e1: IntT => e2: ListT => Cons (e1,e2) : ListT  *)
    | (IntT,ListT) -> ListT
  ...
  | Eq (e1,e2) -> ( match (typeof e1 env, typeof e2 env) with
